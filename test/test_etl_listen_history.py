...
from src.etls.etl_listen_history import HistoryETL
import pandas as pd
...
def test_base_etl_initialization() -> None:
    """
    Test that the ETL instance initializes correctly with given parameters.
    Ensures that the host, port, endpoint, and URL are set as expected.
    Also tests that missing parameters raise a ValueError.
    ...
    assert etl.url == "http://127.0.0.1:8000/listen_history"
